...
| SYS-P1-03 (suma) | **LARGIT** | rambursul se semna ca BOOLEAN, iar SUMA venea din browser si intra direct in cererea catre curier. Auditurile numesc curierii; erau **opt** locuri, fiindca **Woot, Colete si eColet** primeau obiectul `destination` intreg si isi luau singuri `cod` din el. Acum toti primesc `pragulRambursului` = `max(cat cere browserul, valoareMarfii)`, iar la cei trei brokeri campul a fost scos din TIP, ca `tsc` sa enumere apelantii. Masurat: 234 de comenzi cu ramburs, 17 magazine, 214 in 90 de zile, **niciuna atinsa**, fiindca un `max` nu poate cobori nimic. ⚠ INDREPTAT 14.09.2026: aici scria „browserul trimite totalul, pragul e doar marfa", ceea ce e FALS. Amandoua formularele trimit MARFA (`CheckoutForm.tsx:278` cu `total` din `useCart()`, adica suma subtotalurilor de linie din `CartProvider.tsx:356`; `OrderModal.tsx:1335` cu `subtotal`). Cele doua marimi sunt aceeasi marime, deci pragul nu se vede fiindca sunt EGALE, nu fiindca una ar fi mai mare. Codul si masuratoarea raman bune; temeiul scris langa ele era gresit, si plecase din comentariul lui `pragulRambursului` in proba si in randul asta | `5d9f807b` |
