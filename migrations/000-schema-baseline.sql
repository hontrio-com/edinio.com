...
begin
  update public.catalog_produs cp set
    ...
    -- `jsonb_exists(...)`, nu operatorul de intrebare. Sunt acelasi lucru pentru Postgres, dar
    -- semnul e si marca de parametru pentru mai multe drivere (JDBC si altele): trecuta printr-unul
    -- dintre ele, migratia ar fi picat cu "insufficient parameters" pe o instructiune valida.
...
CREATE OR REPLACE FUNCTION public.catalog_murdareste_configurator(p_business uuid, p_configurator uuid, p_categorii text[])
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
  ...
  select p.id, p.business_id
   ...
   where p.business_id = p_business
     and (
       ...
       -- fiindca nimic nu-l mai pune la coada.
...
begin
  if tg_op = 'DELETE' then
    /*
     * Cascada de la magazinul sters trece: `on delete cascade` sterge copiii printr-un
     * declansator AFTER pe parinte, deci cand ajungem aici randul din `businesses` e deja plecat.
     * La o stergere DIRECTA el exista, si exceptia se ridica mai departe.
     */
...
create table if not exists public.catalog_produs (
  ...
  cere_configurare boolean default false not null,
...
create table if not exists public.configurator_componente (
  ...
  nume text not null,
  product_id uuid,
  pret_bucata numeric(10,2) default 0 not null,
...
alter table public.configurator_componente add constraint configurator_componente_pkey PRIMARY KEY (id);
alter table public.configurator_fisiere add constraint configurator_fisiere_pkey PRIMARY KEY (id);
...
alter table public.configurator_componente add constraint configurator_componente_cost_check CHECK (((cost_bucata IS NULL) OR (cost_bucata >= (0)::numeric)));
alter table public.configurator_componente add constraint configurator_componente_nume_check CHECK ((btrim(nume) <> ''::text));
alter table public.configurator_componente add constraint configurator_componente_pret_check CHECK ((pret_bucata >= (0)::numeric));
alter table public.configurator_fisiere add constraint configurator_fisiere_cheie_check CHECK ((btrim(cheie) <> ''::text));
alter table public.configurator_fisiere add constraint configurator_fisiere_mime_check CHECK ((btrim(mime) <> ''::text));
alter table public.configurator_fisiere add constraint configurator_fisiere_octeti_check CHECK (((octeti > 0) AND (octeti <= 26214400)));
...
alter table public.configurator_componente add constraint configurator_componente_business_id_fkey FOREIGN KEY (business_id) REFERENCES businesses(id) ON DELETE CASCADE;
alter table public.configurator_componente add constraint configurator_componente_product_id_fkey FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE SET NULL;
alter table public.configurator_fisiere add constraint configurator_fisiere_business_id_fkey FOREIGN KEY (business_id) REFERENCES businesses(id) ON DELETE CASCADE;
alter table public.configurator_fisiere add constraint configurator_fisiere_comanda_id_fkey FOREIGN KEY (comanda_id) REFERENCES orders(id) ON DELETE SET NULL;
...
CREATE INDEX configurator_componente_magazin_idx ON public.configurator_componente USING btree (business_id, nume);
CREATE INDEX configurator_componente_produs_idx ON public.configurator_componente USING btree (product_id) WHERE (product_id IS NOT NULL);
CREATE INDEX configurator_fisiere_comanda_idx ON public.configurator_fisiere USING btree (comanda_id) WHERE (comanda_id IS NOT NULL);
CREATE INDEX configurator_fisiere_magazin_idx ON public.configurator_fisiere USING btree (business_id, creat_la DESC);
CREATE INDEX configurator_fisiere_orfani_idx ON public.configurator_fisiere USING btree (creat_la) WHERE (comanda_id IS NULL);
...
CREATE TRIGGER set_configurator_componente_updated_at BEFORE UPDATE ON public.configurator_componente FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
create policy owner_all_configurator_categorii on public.configurator_categorii as PERMISSIVE for ALL to public using ((business_id IN ( SELECT businesses.id
  ...
  WHERE (businesses.user_id = ( SELECT auth.uid() AS uid))))) with check (((business_id IN ( SELECT businesses.id
   FROM businesses
  WHERE (businesses.user_id = ( SELECT auth.uid() AS uid)))) AND (EXISTS ( SELECT 1
   FROM configuratoare c
  WHERE ((c.id = configurator_categorii.configurator_id) AND (c.business_id = configurator_categorii.business_id))))));
create policy owner_all_configurator_componente on public.configurator_componente as PERMISSIVE for ALL to public using ((business_id IN ( SELECT businesses.id
   FROM businesses
  WHERE (businesses.user_id = ( SELECT auth.uid() AS uid))))) with check (((business_id IN ( SELECT businesses.id
   FROM businesses
  WHERE (businesses.user_id = ( SELECT auth.uid() AS uid)))) AND ((product_id IS NULL) OR (EXISTS ( SELECT 1
   FROM products p
  WHERE ((p.id = configurator_componente.product_id) AND (p.business_id = configurator_componente.business_id)))))));
create policy owner_select_configurator_fisiere on public.configurator_fisiere as PERMISSIVE for SELECT to public using ((business_id IN ( SELECT businesses.id
   FROM businesses
  WHERE (businesses.user_id = ( SELECT auth.uid() AS uid)))));
create policy owner_all_configurator_produse on public.configurator_produse as PERMISSIVE for ALL to public using ((business_id IN ( SELECT businesses.id
  ...
  WHERE (businesses.user_id = ( SELECT auth.uid() AS uid)))) AND (EXISTS ( SELECT 1
   FROM configuratoare c
  WHERE ((c.id = configurator_produse.configurator_id) AND (c.business_id = configurator_produse.business_id)))) AND (EXISTS ( SELECT 1
   FROM products p
  WHERE ((p.id = configurator_produse.product_id) AND (p.business_id = configurator_produse.business_id))))));
create policy owner_all_configurator_versiuni on public.configurator_versiuni as PERMISSIVE for ALL to public using ((business_id IN ( SELECT businesses.id
  ...
  WHERE (businesses.user_id = ( SELECT auth.uid() AS uid)))) AND (EXISTS ( SELECT 1
   FROM configuratoare c
  WHERE ((c.id = configurator_versiuni.configurator_id) AND (c.business_id = configurator_versiuni.business_id))))));
...
grant DELETE on table public.configurator_componente to authenticated;
grant INSERT on table public.configurator_componente to authenticated;
grant SELECT on table public.configurator_componente to authenticated;
grant UPDATE on table public.configurator_componente to authenticated;
grant DELETE on table public.configurator_componente to service_role;
grant INSERT on table public.configurator_componente to service_role;
grant REFERENCES on table public.configurator_componente to service_role;
grant SELECT on table public.configurator_componente to service_role;
grant TRIGGER on table public.configurator_componente to service_role;
grant TRUNCATE on table public.configurator_componente to service_role;
grant UPDATE on table public.configurator_componente to service_role;
grant DELETE on table public.configurator_fisiere to service_role;
grant INSERT on table public.configurator_fisiere to service_role;
grant REFERENCES on table public.configurator_fisiere to service_role;
grant SELECT on table public.configurator_fisiere to service_role;
grant TRIGGER on table public.configurator_fisiere to service_role;
grant TRUNCATE on table public.configurator_fisiere to service_role;
grant UPDATE on table public.configurator_fisiere to service_role;
...
grant SELECT (business_id) on table public.configurator_fisiere to authenticated;
grant SELECT (comanda_id) on table public.configurator_fisiere to authenticated;
grant SELECT (creat_la) on table public.configurator_fisiere to authenticated;
grant SELECT (id) on table public.configurator_fisiere to authenticated;
grant SELECT (inaltime) on table public.configurator_fisiere to authenticated;
grant SELECT (latime) on table public.configurator_fisiere to authenticated;
grant SELECT (mime) on table public.configurator_fisiere to authenticated;
grant SELECT (nume) on table public.configurator_fisiere to authenticated;
grant SELECT (octeti) on table public.configurator_fisiere to authenticated;
...
grant execute on function public.catalog_murdareste_configurator(p_business uuid, p_configurator uuid, p_categorii text[]) to service_role;
...
revoke execute on function public.catalog_murdareste_configurator(p_business uuid, p_configurator uuid, p_categorii text[]) from public;
