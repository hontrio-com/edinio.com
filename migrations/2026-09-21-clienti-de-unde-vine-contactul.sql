-- ═══════════════════════════════════════════════════════════════════════════
-- CLIENTI, G1 (urmare): de unde vine contactul                 (21.09.2026)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- ⚠⚠ ADAUGAREA DE MANA A FACUT O ETICHETA SA MINTA, CHIAR IN ZIUA IN CARE A
-- FOST SCRISA. Badge-ul „Importat" se punea dupa `orderCount === 0`, fiindca
-- pana azi tot ce n-avea comenzi venea dintr-un import — o deducere adevarata
-- exact cat timp nu exista al doilea drum.
--
-- Vazut pe ecran, imediat: un client adaugat de mana de la telefon aparea in
-- lista cu eticheta „Importat". Nicio eroare, nicio proba cazuta; doar o
-- propozitie falsa despre un om, pe care comerciantul n-avea cum s-o dezminta.
--
-- ⚠ Si asta e felul de defect care se naste intotdeauna asa: o insusire dedusa
-- dintr-alta, corecta pana cand cineva adauga a doua cale. De-aia acum se poarta
-- CHIAR insusirea (`customers.source`), nu una din care se ghiceste ea.
--
-- ⚠ `source` e NULL pentru un cumparator: el n-are rand in `customers`, e o
-- grupare peste comenzile lui. Si asta e raspunsul corect — „de nicaieri
-- anume, a comandat".
--
-- ⚠ FACE CU PUTINTA filtrul „adaugat manual", pe care `lib/customers/filtre.ts`
-- il lasase deoparte scriind „nu exista adaugarea manuala". Acum exista, dar
-- filtrul NU s-a adaugat aici: ar fi cerut inca un argument la
-- `customer_in_segment`, deci stergerea si recrearea ei, a listei si a
-- numaratorii — pentru ceva ce pe productie are azi zero randuri. Se face cand
-- se aduna oameni adaugati de mana, nu inainte.

/*
  ⚠ AMANDOUA SE STERG INTAI, si in ordinea asta. Postgres refuza sa schimbe
  lista de coloane intoarse de o functie printr-un `create or replace`, iar
  `customers_aggregate` o cheama pe `customers_merged`: lista se sterge prima,
  altfel raman doua functii care nu se mai potrivesc.

  ⚠ Semnatura ARGUMENTELOR nu se schimba la niciuna, deci niciun apelant nu se
  atinge — numai o coloana in plus in ce se intoarce.
*/
...
-- ── Lista, cu o coloana in plus ────────────────────────────────────────────
--
-- Stearsa mai sus, impreuna cu `customers_merged`.
