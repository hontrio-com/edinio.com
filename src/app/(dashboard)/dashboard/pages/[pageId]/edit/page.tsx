...
export default async function EditCustomPage({ params }: { params: Promise<{ pageId: string }> }) {
  ...
  const [{ data: productsRaw }, { data: cats }, { data: formsRaw }] = await Promise.all([
    ...
    supabase.from("forms").select("id, name, fields, submit_label, success_message, email_enabled, email_to, mailchimp_enabled")
  ...
  const forms: FormDef[] = (formsRaw ?? []).map((f) => ({
    ...
    email_enabled: f.email_enabled, email_to: f.email_to, mailchimp_enabled: f.mailchimp_enabled,
  }));
