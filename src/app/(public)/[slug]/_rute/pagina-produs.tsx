...
/*
 * ⚠ AICI STA PAGINA DE PRODUS, nu in `product/[productSlug]/page.tsx` (25.09.2026).
 *
 * O randeaza doua rute: cea cu segmentul implicit (`/product/<slug>`) si cea cu
 * prefixul ales de magazin din Setari > Permalink-uri (`/<prefix>/<slug>`, prin
 * `[...rest]`). Un `page.tsx` nu are voie sa exporte altceva decat ce stie Next,
 * deci implementarea e aici, intr-un dosar privat (`_rute` nu e ruta).
 */
interface Props {
  slug: string;
  productSlug: string;
...
export async function metadataProdus({ productSlug, slug }: Props): Promise<Metadata> {
  ...
  const [{ customDomain, storeMode }, permalinkuri] = await Promise.all([
    getBusinessMetaCached(slug), setareaPermalinkurilorMagazinului(slug),
  ]);
  const url = hrefProdus(storeBaseUrl({ slug, custom_domain: customDomain }), canonicalSlug, permalinkuri.produs);
...
export async function PaginaProdus({ slug, productSlug }: Props) {
  const supabase = await createClient();
  ...
  if (UUID_RE.test(productSlug) && product.slug) {
    // Cu prefixul CURENT al magazinului (Setari > Permalink-uri), pe gazda cererii.
    redirect(await adresaCurenta(slug, "produs", [product.slug]));
  }
  const { produs: prefixProdus } = await setareaPermalinkurilorMagazinului(slug);
  ...
  */
  after(async () => {
    ...
    await scrieEvenimentAnalitic({
      ...
      path: hrefProdus("", product.slug ?? productSlug, prefixProdus),
  ...
  const productUrl = hrefProdus(storeBase, product.slug ?? productSlug, prefixProdus);
