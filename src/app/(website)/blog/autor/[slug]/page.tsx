 ...
 */
async function autorulSiArticolele(slug: string) {
  const autor = await autorDupaSlug(slug);
  if (!autor) return null;
  const articole = await articoleleAutorului(autor.id);
  return { autor, articole };
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const gasit = await autorulSiArticolele(slug);
  if (!gasit || gasit.articole.length === 0) return { title: "Autor negăsit" };

  const { autor, articole } = gasit;
  return siteMetadata({
    title: `${autor.name}${autor.role_title ? `, ${autor.role_title}` : ""} | Blog Edinio`,
    description:
      autor.bio?.trim() ||
      `${articole.length} ${articole.length === 1 ? "articol scris" : "articole scrise"} de ${autor.name} pe blogul Edinio.`,
    path: `/blog/autor/${autor.slug}`,
  });
}

export default async function AutorBlogPage({ params }: Props) {
  const { slug } = await params;
  const gasit = await autorulSiArticolele(slug);
  if (!gasit || gasit.articole.length === 0) notFound();
      ...
      <section className="mx-auto max-w-[1140px] px-5 pb-20">
        ...
        <div className="mt-10 flex items-center justify-between">
          <h2 className="text-[20px] font-semibold text-ink">
            {articole.length === 1 ? "Un articol" : `${articole.length} articole`}
          </h2>
