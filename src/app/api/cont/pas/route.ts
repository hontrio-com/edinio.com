...
import {
  anuntaParolaSchimbata, incheieIntrarea, mesajulPasului, MESAJ_PARTEA_INTAI, retrimitePas,
  tineMinteDispozitivul, verificaPas,
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (corp?.actiune === "retrimite") {
      ...
      if (r.motiv === "fara-provocare" || r.motiv === "provocare-incheiata") {
        return NextResponse.json({ eroare: "Pasul a expirat. Reia de la inceput." }, { status: 400 });
      }
    ...
    if (!r.ok || !r.contId) {
      return NextResponse.json({ eroare: mesajulPasului(r.motiv) }, { status: 400 });
    }
