  ...
  const marcheazaVerificat = async (o: Comanda, tipNou: string | null, codNou: string | null) => {
    const { error } = await admin
      ...
      .update({
         ...
         * cu cea din baza, in afara de cazul in care nu mai trebuia scrisa deloc.
         */
        ...(tipNou !== null ? { ups_status_type: tipNou } : {}),
        ...(codNou !== null ? { ups_status_code: codNou } : {}),
    ...
    for (let i = 0; i < comenzi.length; i++) {
      ...
      if (tinta) {
        const rez = await tranzitieComandaMarketplace(admin, {
          ...
          status: tinta,
          sursa: "ups",
          expediere: { coloana: "ups_awb_number", valoare: o.ups_awb_number },
        });
        if (rez === "ok") {
          mutate++;
