...
export function KlaviyoClient({ businessId, initialConfig }: { businessId: string; initialConfig: KlaviyoPublicConfig }) {
  ...
  function connect() {
    ...
    startConnect(async () => {
      const res = await connectKlaviyo(businessId, apiKey.trim());
  ...
  function reloadLists() {
    startLists(async () => {
      const res = await getKlaviyoLists(businessId);
      if ("error" in res) { toast.error(res.error); return; }
      setLists(res.lists);
      setListsLoaded(true);
  ...
  function save() {
    startSave(async () => {
      ...
      const res = await saveKlaviyoSettings(businessId, {
  ...
  function disconnect() {
    startSave(async () => {
      const res = await disconnectKlaviyo(businessId);
      if ("error" in res) { toast.error(res.error); return; }
