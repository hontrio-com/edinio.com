  ...
  if (!deschis) {
    return (
      <div className="space-y-3">
        {gata && (
          <p role="status" className="text-sm text-[var(--st-text)]">
            Parola a fost schimbata. Celelalte dispozitive au iesit din cont.
          </p>
        )}
        {!areParola && !gata && (
    ...
    <form onSubmit={trimite} className="max-w-md space-y-4">
      {(areParola || gata) && (
        <div>
          ...
          <input id="parola-veche" type={tip} autoComplete="current-password" value={veche}
      ...
      <button type="button" onClick={() => setVizibile((v) => !v)} aria-pressed={vizibile}
        className={`inline-flex items-center gap-1.5 rounded-sm text-sm text-[var(--st-muted)] hover:text-[var(--st-text)] ${FOCUS}`}>
