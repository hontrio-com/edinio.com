...
/**
 * Banda de categorii, UNA SINGURA pentru toate paginile care o arata.
 *
 * Sta aici, si nu in fiecare model de pagina, fiindca se afisa in doua feluri:
 * pe prima pagina cercuri cu imagini, pe pagina de catalog pastile text. Nu era
 * o alegere de design — erau doua implementari scrise la momente diferite, iar
 * cea de a doua nu stia de imagini. La un magazin ca Vetdepo, unde toate cele 12
 * raioane si cateva subcategorii au poza, jumatate din magazin arata pozele si
 * cealalta jumatate nu.
 *
 * Forma se alege AUTOMAT DIN DATE, nu din setari: cercuri cand macar o categorie
 * are imagine, pastile altfel. Asa un magazin fara poze ramane exact cum era.
 *
 * Ambele forme sunt ierarhice: apasarea unei categorii cu subcategorii intra in
 * ea, iar controlul din fata devine „Inapoi".
 *
 * Categoriile sunt ancore, nu butoane: filtrarea se face pe loc, fara navigare,
 * deci fara ele pagina n-ar avea niciun link crawlabil catre vreo categorie (in
 * headere linkurile apar abia dupa deschiderea dropdown-ului, iar footerul
 * „centered" nu arata categorii deloc). Adresa se compune in
 * `lib/storefront/category-href`, impreuna cu celelalte sase locuri care arata
 * aceleasi categorii.
 */
...
export function NavigareCategorii({ className }: { className?: string }) {
  const { hasCategories, hasAnyCategoryImage } = useStorefront();
  if (!hasCategories) return null;
  return hasAnyCategoryImage
    ? <CerculeteCuImagini className={className} />
    : <PastileText className={className} />;
}

function PastileText({ className }: { className?: string }) {
  const { categoriiRoot, categoriiPePagina, color, categoryFilter, currentCategoryItems, isDrilled, drillParentName, selectCategoryItem, resetCategory, goBackCategory } =
    useStorefront();
  ...
  return (
    <CategoryScroller className={className}>
...
function CerculeteCuImagini({ className }: { className?: string }) {
  const { categoriiRoot, categoriiPePagina, color, categoryFilter, currentCategoryItems, isDrilled, drillParentName, selectCategoryItem, resetCategory, goBackCategory } =
    useStorefront();
