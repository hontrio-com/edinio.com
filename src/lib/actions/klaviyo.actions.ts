...
export async function syncExistingCustomers(
  ...
  const orders = await fetchAllRowsStrict("klaviyo.syncExistingCustomers.orders", (from, to) =>
    owned.supabase
      .from("orders")
      .select("customer_email, order_source")
  ...
  const emails = Array.from(new Set(
    orders
       ...
       * schimba interogarea de mai sus si scoate filtrul fara sa se uite incoace. Aici mai
       * atarna ceva: mai jos e `subscribeProfiles`, adica ABONARE la marketing, nu o simpla
       * scriere de contact.
       */
      .filter((o) => !clientDeMarketplace(o.order_source))
