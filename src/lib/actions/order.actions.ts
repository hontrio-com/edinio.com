...
import { maybeSyncMailchimpSubscriber, orderValueTag } from "@/lib/mailchimp-sync";
...
const CHEI_ATRIBUIRE = [
  ...
  "ttp",
  /* Mailchimp, din 18.09.2026: campania si codul de urmarire din link, pentru venitul atribuit campaniei. */
  "mc_cid", "mc_tc",
] as const;
  ...
  */
  ...
  try {
    ...
    if (settings) {
      ...
      /*
       * EMAIL MARKETING (Mailchimp, Brevo, Klaviyo): abonarea clientului care a bifat newsletterul.
       *
       * ⚠ COMANDA INSASI NU MAI PLEACA DE AICI. O scrie in coada triggerul de pe `orders`
       * (`lib/email-marketing/coada.ts`), impreuna cu plata, expedierea, livrarea, anularea si
       * rambursarea, iar `/api/cron/email-marketing` o trimite, cu reincercari.
       *
       * ⚠ PRIN `dupaRaspuns`, NU `void`: actiunea raspunde imediat, iar o abonare pornita si uitata
       * putea fi taiata cand functia ingheata dupa raspuns, fara nicio urma.
       */
...
export async function updateOrder(orderId: string, data: { status: string; payment_status: string; awb?: string }) {
  ...
  */
  ...
  /*
   * ⚠ PLATA, ANULAREA SI RAMBURSAREA NU SE MAI ANUNTA DE AICI catre Mailchimp, Brevo si Klaviyo:
   * le scrie in coada triggerul de pe `orders` (`lib/email-marketing/coada.ts`), pe TOATE caile, cu
   * reincercari. Poarta de marketplace sta in cititorul fiecarui furnizor (`clientDeMarketplace`).
   */
  ...
  try {
    ...
    if (settings) {
      ...
      const emailPayload = {
        ...
        // comerciantului nu se aduna cu lista de deasupra lui. Vezi `BaniComanda`.
        //
        // ⚠ Si INSTANTANEUL personalizarii, cand linia il are — aceeasi forma ca in `placeOrder`.
        // Fara el atelierul primeste „Fototapet personalizat x1 — 1.234,00 lei” si trebuie sa deschida
        // panoul ca sa afle ce are de tiparit: dimensiunile, textul scris de client si numarul de
        // fisiere sunt toate in comanda. Acelasi `emailPayload` pleaca si catre CLIENT, care trebuie
        // sa poata confirma ce a comandat inainte sa intre in productie.
        //
        // ⚠ CE A COSTAT LIPSA LUI: randul asta a stat aici fara spread desi `placeOrder` il avea,
        // deci comanda pusa in cos si trimisa din checkout ajungea la atelier ca „nume x1” si atat —
        // tocmai calea pe care merg cei mai multi cumparatori. De cand emailul nu mai poarta nici
        // adresele fisierelor, instantaneul e SINGURUL lucru care mai iese despre ce s-a comandat.
        //
        // ⚠ Spread CONDITIONAT: `allItems` e o uniune de forme, iar extraoptiunile n-au campul deloc.
        items: allItems.map(i => ({
          product_id: i.product_id, name: i.name, quantity: i.quantity, price: i.price,
          ...("customization" in i ? { customization: i.customization } : {}),
        })),
