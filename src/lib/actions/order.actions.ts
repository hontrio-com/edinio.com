...
// TEMP: manual re-send of the "Comanda noua" notification to the store's current
// notification email. Used to recover a notification that went to an old address.
export async function resendOrderNotification(
  ...
  const { data: biz } = await admin
    .from("businesses").select("id, business_name, user_id").eq("id", order.business_id).single();
  if (!biz || biz.user_id !== user.id) return { error: "Acces interzis" };

  const { data: settings } = await admin
    .from("store_settings").select("notifications_config").eq("business_id", order.business_id).single();
  const cfg = parseNotificationsConfig((settings?.notifications_config as Record<string, unknown>) ?? {});

  let notifyEmail = cfg.notification_email;
  if (!notifyEmail) {
    const { data: authData } = await admin.auth.admin.getUserById(biz.user_id);
    notifyEmail = authData?.user?.email ?? "";
  }
  if (!notifyEmail) return { error: "Nu exista un email de notificare configurat." };

  const addr = (order.shipping_address ?? {}) as {
    address?: string; city?: string; county?: string; courier_label?: string; delivery_type?: string; locker_name?: string;
  };
  const custom = order.notes && typeof order.notes === "object"
    ? (order.notes as unknown as Record<string, string>)
    : null;

  try {
    await sendNewOrderEmail(notifyEmail, {
      order_number: order.order_number,
      customer_name: order.customer_name,
      ...
      vat_amount: order.vat_amount != null ? Number(order.vat_amount) : undefined,
      ...
      courier_label: addr.courier_label ?? null,
      delivery_type: addr.delivery_type ?? null,
      locker_name: addr.locker_name ?? null,
      custom_fields: custom,
    });
  } catch (e) {
    logError({ action: "resendOrderNotification", message: (e as Error).message ?? "send failed", details: { orderId }, userId: user.id, severity: "warning" });
    return { error: "Trimiterea emailului a esuat." };
  }

  return { success: true, sentTo: notifyEmail };
}
