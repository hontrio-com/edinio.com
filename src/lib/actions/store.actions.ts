...
export async function getPublicStoreConfig(businessId: string): Promise<{
  ...
  const { data } = await admin
    .from("store_settings")
    .select("page_content, vat_enabled, vat_rate, prices_include_vat, show_vat_breakdown, shipping_zones, min_order_amount, stripe_config, netopia_config, ipay_config, dpd_config, payment_methods, card_discount_config, mailchimp_config, brevo_config, klaviyo_config")
