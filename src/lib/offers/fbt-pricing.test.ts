...
/*
  ⚠ Ajutoarele astea PASTREAZA probele de ieri neatinse in intelesul lor: de cand
  un set poate cere cantitati, functiile primesc `{ pret, bucati }` in loc de
  numere, iar `bucati` lipsa inseamna o bucata. Deci fiecare cifra de mai jos e
  aceeasi cifra ca inainte de cantitati — asta si probeaza ca nimic nu s-a mutat.
*/
const l = (pret: number, bucati?: number) => ({ pret, bucati });
const ll = (preturi: number[]) => preturi.map((x) => l(x));
...
test("cazul LIVE de la BricoSmart: companionii nu mai ies pe degeaba", () => {
  ...
  const rezultat = fbtCompanionPrices(l(1244.88), ll(companioni), procent(10));
...
test("ancora scumpa nu mai poate duce companionii la zero, oricat ar creste", () => {
  ...
  for (const ancora of [100, 1000, 10_000, 1_000_000]) {
    const r = fbtCompanionPrices(l(ancora), ll(companioni), procent(20));
...
test("procentul se aplica pe companioni, indiferent de ancora", () => {
  assert.equal(sumaRotunjita(fbtCompanionPrices(l(50), ll([50]), procent(10))), 45);
  assert.equal(sumaRotunjita(fbtCompanionPrices(l(500), ll([50]), procent(10))), 45);
});

test("economia se imparte proportional intre companioni", () => {
  const r = fbtCompanionPrices(l(100), ll([60, 40]), procent(50));
...
test("reducerea in suma fixa se imparte tot pe cota companionilor", () => {
  ...
  assert.equal(sumaRotunjita(fbtCompanionPrices(l(200), ll([100]), suma(60))), 80);
});

test("nimic nu coboara sub zero, nici la reduceri absurde", () => {
  const r = fbtCompanionPrices(l(0), ll([10, 5]), procent(100));
...
test("fara reducere, companionii raman la pretul lor", () => {
  ...
  assert.deepEqual(fbtCompanionPrices(l(100), ll([30, 20]), fara), [30, 20]);
});

test("fara companioni nu se prabuseste", () => {
  assert.deepEqual(fbtCompanionPrices(l(100), ll([]), procent(10)), []);
});

test("companionii nu cresc niciodata peste pretul lor", () => {
  for (const ancora of [0, 10, 500]) {
    const r = fbtCompanionPrices(l(ancora), ll([25, 75]), procent(15));
...
test("ce se afiseaza si ce se incaseaza dau acelasi pret de set", () => {
  ...
  for (const [ancora, companioni, pct] of cazuri) {
    const incasat = fbtCompanionPrices(l(ancora), ll(companioni), procent(pct));
    ...
    const afisat = imparteEconomiaCompanionilor(l(ancora), ll(companioni), economieSet);
