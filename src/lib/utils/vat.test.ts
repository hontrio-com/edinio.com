...
const gol = { goods: 0, extras: 0, discount: 0, cardDiscount: 0, codDiscount: 0, codFee: 0 };
...
test("acelasi total, oricine il calculeaza", () => {
  const p = { goods: 249.9, extras: 12.5, discount: 25, cardDiscount: 4.5, codDiscount: 0, codFee: 4.99 };
  const shipping = 19.9;

  const baza = vatBase(p);
  const { vatAddOn } = computeVat(baza, FARA_TVA);
  const total = Math.max(0, Math.round(
    (p.goods + p.extras - p.discount - p.cardDiscount - p.codDiscount + p.codFee + shipping + vatAddOn) * 100,
  ) / 100);

  // Baza = 249,90 + 12,50 - 25 - 4,50 + 4,99 = 237,89; TVA 19% = 45,20.
  assert.equal(baza, 237.89);
  assert.equal(vatAddOn, 45.2);
  assert.equal(total, 302.99);
});
